...
import { Database, Activity, Code, Gamepad2, Settings, Terminal, Zap, Compass, Cpu, Network, Sparkles, ShieldAlert, FlaskConical } from 'lucide-react';

export const PANELS = [
  { id: 'revenue_dashboard', label: 'Revenue & Orchestration', icon: <Sparkles className="w-5 h-5 text-yellow-400" />, isMain: true },
  { id: 'data_dominance', label: 'Data Dominance & Arbitrage', icon: <Database className="w-5 h-5 text-blue-500" />, isMain: true },
  { id: 'ip_generation', label: 'Generative IP Exploitation', icon: <Sparkles className="w-5 h-5 text-purple-500" />, isMain: true },
  { id: 'game_economy', label: 'Autonomous Game Economy', icon: <Zap className="w-5 h-5 text-amber-500" />, isMain: true },
  { id: 'god_mode_os', label: 'God Mode OS', icon: <Terminal className="w-5 h-5 text-emerald-400" />, isMain: true },
  { id: 'experimental_feedback', label: 'Experimental Feedback Loop', icon: <FlaskConical className="w-5 h-5 text-emerald-400" />, isMain: true },
  { id: 'prime_ide', label: 'Prime Architect IDE', icon: <Terminal className="w-5 h-5 text-cyan-400" />, isMain: false },
  { id: 'code_surgeon', label: 'Code Surgeon', icon: <Cpu className="w-5 h-5 text-pink-400" />, isMain: false },
  { id: 'nexus_os', label: 'Nexus Operating System', icon: <Terminal className="w-5 h-5 text-emerald-400" />, isMain: false },
  { id: 'cognitive_nexus', label: 'Cognitive Nexus', icon: <Sparkles className="w-5 h-5 text-purple-400" />, isMain: false },
  { id: 'spatial_command', label: 'Spatial Command', icon: <Compass className="w-5 h-5 text-cyan-400" />, isMain: false },
  { id: 'motorola_hardware', label: 'Motorola Hardware', icon: <Terminal className="w-5 h-5 text-emerald-400" />, isMain: false },
  { id: 'motorola_grid', label: 'Motorola Data Grid', icon: <Network className="w-5 h-5 text-emerald-500" />, isMain: false },
  { id: 'fractal_expansion', label: 'Fractal Expansion', icon: <Sparkles className="w-5 h-5 text-emerald-400" />, isMain: false },
  { id: 'myth_engine', label: 'The Myth Engine', icon: <Database className="w-5 h-5 text-blue-500" />, isMain: false },
  { id: 'reality_compiler', label: 'Reality Compiler', icon: <Terminal className="w-5 h-5 text-purple-500" />, isMain: false },
  { id: 'neurospatial_wormhole', label: 'Neurospatial Wormhole', icon: <Sparkles className="w-5 h-5 text-indigo-400" />, isMain: false },
  { id: 'evolution_roadmap', label: 'Evolution Roadmap', icon: <Compass className="w-5 h-5 text-cyan-400" />, isMain: false },
  { id: 'quantum_chat', label: 'Quantum Chat & Strategy', icon: <Zap className="w-5 h-5 text-pink-500" />, isMain: true },
  { id: 'ouroboros_engine', label: 'Ouroboros Engine', icon: <Cpu className="w-5 h-5 text-emerald-500" />, isMain: true },
  { id: 'bannon_game', label: 'Bannon Sandbox', icon: <Gamepad2 className="w-5 h-5 text-red-500" />, isMain: true },
  { id: 'game_studio', label: 'Developer Studio', icon: <Settings className="w-5 h-5 text-yellow-500" />, isMain: true },
  { id: 'moveset_db', label: 'Moveset Matrix', icon: <Database className="w-5 h-5 text-blue-400" />, isMain: false },
  { id: 'physics_lab', label: 'Physics Laboratory', icon: <Activity className="w-5 h-5 text-emerald-500" />, isMain: false },
  { id: 'ai_assistant', label: 'AI Diagnostician', icon: <Code className="w-5 h-5 text-indigo-400" />, isMain: false },
  { id: 'system_overseer', label: 'System Overseer', icon: <Compass className="w-5 h-5 text-purple-400" />, isMain: false },
  { id: 'kinetic_grid', label: 'Kinetic Grid', icon: <Activity className="w-5 h-5 text-emerald-500" />, isMain: false },
  { id: 'swarm_routing', label: 'Swarm Routing', icon: <Network className="w-5 h-5 text-emerald-500" />, isMain: false },
  { id: 'abundance_matrix', label: 'Abundance Matrix', icon: <Sparkles className="w-5 h-5 text-fuchsia-500" />, isMain: false },
  { id: 'security_vault', label: 'Security Vault', icon: <ShieldAlert className="w-5 h-5 text-red-500" />, isMain: false },
  { id: 'apex_system', label: 'Apex System Logic', icon: <Cpu className="w-5 h-5 text-indigo-500" />, isMain: false },
  { id: 'metaconscious', label: 'Metaconscious Apotheosis', icon: <Sparkles className="w-5 h-5 text-purple-400" />, isMain: false }
];

export default function App() {
  const [activePanel, setActivePanel] = useState('revenue_dashboard');
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [globalSelectedFile, setGlobalSelectedFile] = useState<string>(() => {
    try { return localStorage.getItem('globalSelectedFile') || ''; } catch (e) { return ''; }
  });

  useEffect(() => {
    if (globalSelectedFile) {
      try { localStorage.setItem('globalSelectedFile', globalSelectedFile); } catch (e) {}
    }
  }, [globalSelectedFile]);
    ...
    if (!globalSelectedFile) {
      fetch('/api/library').then(r => r.json()).then(data => {
        if (data.files && data.files.length > 0) {
          ...
          const core = sortedFiles.find((f: any) => 
    ...
    <div className="pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)] min-h-[100dvh] bg-[#030303] text-white font-mono flex flex-col overflow-hidden select-none">
      ...
      <div className="flex flex-1 relative overflow-hidden">
        ...
        <main className="flex-1 flex flex-col relative overflow-y-auto p-4 transition-all duration-300 z-10 w-full min-w-0">
          ...
          {activePanel === 'god_mode_os' && <GodModeOS onNavigate={(id) => setActivePanel(id)} />}
          {activePanel === 'experimental_feedback' && <ExperimentalFeedbackTerminal />}
          ...
          {activePanel === 'system_overseer' && <SystemOverseer><div className="flex h-full w-full items-center justify-center p-8"><div className="text-gray-400 font-mono text-xs max-w-lg bg-[#111] p-6 border border-[#333] rounded-xl"><span className="text-cyan-500 font-bold mb-4 block">SYSTEM OVERSEER ONLINE</span>Scanning local telemetry layers...<br/><br/>The active God Mode OS and React error boundary are stable. Zero sub-routine desyncs detected.</div></div></SystemOverseer>}
          {activePanel === 'kinetic_grid' && <KineticVisualizer />}
          {(activePanel === 'swarm_routing' || activePanel === 'swarm_monitor') && <SwarmMonitor />}
          {(activePanel === 'abundance_matrix' || activePanel === 'abundance_compiler') && <AbundanceCompiler />}
          {activePanel === 'security_vault' && <SecurityVault />}
          {(activePanel === 'apex_system' || activePanel === 'apex_core') && <ApexCoreOS />}
          {(activePanel === 'metaconscious' || activePanel === 'metaconscious_apotheosis') && <MetaconsciousApotheosis />}
          ...
          {(activePanel === 'prime_ide' || activePanel === 'prime_architect') && <PrimeArchitectIDE />}
          ...
          {(activePanel === 'motorola_hardware' || activePanel === 'hardware_sensor') && <MotorolaHardwareBridge />}
          ...
          {activePanel === 'neuroplasticity_engine' && <NeuroplasticityEngine />}
          {activePanel === 'environmental_os' && <EnvironmentalControls />}
          {activePanel === 'earth_observation' && <EarthObservationGrid />}
          {activePanel === 'cellular_rejuvenation' && <CellularRejuvenationMatrix />}
          ...
          {!['revenue_dashboard', 'data_dominance', 'ip_generation', 'game_economy', 'quantum_chat', 'god_mode_os', 'ouroboros_engine', 'bannon_game', 'game_studio', 'moveset_db', 'physics_lab', 'ai_assistant', 'code_surgeon', 'system_overseer', 'kinetic_grid', 'swarm_routing', 'abundance_matrix', 'security_vault', 'apex_system', 'metaconscious', 'autonomous_sandbox', 'kinetic_wealth', 'razor_monitor',
            'prime_ide', 'prime_architect', 'nexus_os', 'cognitive_nexus', 'spatial_command', 'motorola_hardware', 'hardware_sensor', 'motorola_grid', 'fractal_expansion', 'myth_engine', 'reality_compiler', 'neurospatial_wormhole', 'evolution_roadmap', 'omniverse_hyper', 'quantum_reality', 'mlab_daw', 'dashboard_overview', 'sector_matrix', 'omni_singularity', 'neuroplasticity_engine', 'environmental_os', 'earth_observation', 'cellular_rejuvenation', 'master_blueprint', 'environmental_tech', 'embodied_physical', 'one_bit_compression', 'agentic_swarm', 'predictive_reality', 'kinetic_haptic', 'tomodachi_simulation', 'the_crucible', 'biological_override', 'influence_arc', 'kinetic_resistance', 'orbital_resonance', 'activity_logs', 'temporal_memory', 'forge_studio', 'media_forge', 'directive_vault', 'semantic_memory', 'system_settings', 'stability_monitor', 'swarm_monitor', 'swarm_routing', 'system_health', 'void_monitor', 'world_model', 'abundance_compiler', 'apex_core', 'metaconscious_apotheosis'
            ...
            <div className="flex-1 flex items-center justify-center p-8">
              <div className="text-gray-400 font-mono text-sm max-w-lg bg-[#111] p-6 border border-[#333] rounded-xl text-center">
                ...
                The `{activePanel}` sub-routine currently has no dedicated execution context. <br/><br/>
                Please rely on the Cognitive Nexus or God Mode OS for cross-referencing capabilities.
              </div>
            </div>
          )}
        </main>
