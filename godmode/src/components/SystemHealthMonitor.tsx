...
export function SystemHealthMonitor() {
  ...
  const fetchData = async () => {
    try {
      ...
      const storageReq = await fetch("/api/storage/metrics");
      if (storageReq.ok) setStorageStats((await storageReq.json()).stats);
  ...
  return (
    <div className="bg-[#050505] border-b border-white/5 py-2 px-4 flex flex-wrap items-center justify-between text-[10px] font-mono shrink-0">
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-emerald-400 font-bold">
            {routerStats?.activeProvider || "Local Llama"}
          </span>
        </div>
        {/* Costs & Budget */}
        <div className="flex items-center gap-2">
          ...
          <span className="text-amber-400">
            ${routerCosts?.today.toFixed(2) || "0.00"}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-blue-400">
            {sessionVal
              ? new Date(sessionVal.timestamp + "Z").toLocaleTimeString()
              : "AWAITING SNAPSHOT"}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-400">STATE VECTOR:</span>
          <span className="text-indigo-400 font-bold">8-QUBIT LOCAL</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-blue-400">
            {storageStats
              ? `${storageStats.usedSpaceMB}MB / ${storageStats.totalSpaceMB}MB`
              : "CALCULATING..."}
          </span>
        </div>
      </div>
      <div className="flex items-center gap-4">
        <button
          className="flex items-center gap-1 hover:text-emerald-400 transition-colors text-gray-500"
          onClick={fetchData}
        >
