...
export class SemanticSearch {
  ...
  static init() {
    ...
    setTimeout(() => {
      this.indexAll().catch((err) =>
        console.error("[SemanticSearch] Initial sweep failed:", err),
      );
    }, 3000);
  }
  ...
  static async indexAll(): Promise<IndexReport> {
    ...
    const tables = [
      {
        name: "swarm_results",
        mapText: (row: any) =>
          row.result || row.builder_output || row.optimizer_synthesis || "",
      },
      {
        name: "mind_log",
        mapText: (row: any) =>
          row.think_output ||
          [row.directive_intent, row.reasoning].filter(Boolean).join("\n") ||
          "",
      },
      {
        name: "kinetic_logs",
        mapText: (row: any) =>
          row.event_description || row.stage_transition || "",
      },
      {
        name: "dna_archive",
        mapText: (row: any) =>
      ...
      },
      {
        ...
        mapText: (row: any) => row.message_content || row.messages || "",
      },
      {
        name: "overnight_log",
        mapText: (row: any) => (row.task || "") + "\n" + (row.result || ""),
      },
    ...
    for (const tbl of tables) {
      try {
        const rows = memoryVault
          .prepare(`SELECT * FROM ${tbl.name}`)
          .all() as any[];
        tablesIndexed++;
        for (const row of rows) {
          ...
          const existing = memoryVault
            .prepare(`SELECT content_hash FROM semantic_index WHERE id = ?`)
            .get(uuid) as any;
        ...
        console.warn(
          `[SemanticSearch] Table '${tbl.name}' indexing skipped or failed:`,
          err.message,
        );
      }
    }
    ...
    const engine = stats.fallbackModeActive
      ? "TF-IDF Fallback"
  ...
  static async search(
    ...
    try {
      ...
      // Compute dataset stats for BM25 Fallback/Hybrid Ranking
      let totalDocLength = 0;
      const docFrequency = new Map<string, number>();
      const parsedDocs: { entry: any; words: string[]; embedding: number[] }[] =
        [];
      ...
      for (const entry of allEntries) {
        ...
        for (const w of uniqueWords) {
          docFrequency.set(w as string, (docFrequency.get(w as string) || 0) + 1);
        }

        parsedDocs.push({ entry, words, embedding });
      }

      const totalDocs = parsedDocs.length;
      ...
      for (const doc of parsedDocs) {
        const vectorScore = EmbeddingEngine.cosineSimilarity(
          queryEmbedding,
          doc.embedding,
        );
        const bm25Score = EmbeddingEngine.bm25Similarity(
          queryWords,
          doc.words,
          avgDocLength,
          totalDocs,
        ...
        // Advanced Hybrid Ranking Score (normalized)
        // Cosine is -1 to 1, BM25 is unbound but usually 0-20. We will scale BM25 down.
        const hybridScore = vectorScore + bm25Score * 0.05;

        if (hybridScore >= minScore) {
          scoredResults.push({
            sourceTable: doc.entry.source_table,
            sourceId: doc.entry.source_id,
            content: doc.entry.text_content || "",
            score: hybridScore,
            timestamp: doc.entry.indexed_at || new Date().toISOString(),
            tokenCount:
              doc.entry.token_count ||
              Math.ceil((doc.entry.text_content || "").length / 4),
          });
        }
      }
  ...
  static async searchForContext(
    taskIntent: string,
    tokenBudget: number,
  ): Promise<ContextPackage> {
    const allMatches = await this.search(taskIntent, {
      minScore: 0.2,
      limit: 50,
    });
    ...
    for (const match of allMatches) {
      const matchTokens =
        match.tokenCount || Math.ceil(match.content.length / 4);
    ...
    if (results.length === 0) {
      ...
      console.log(
        `[SemanticSearch] 0 semantic results for taskIntent '${taskIntent}'. Entering keyword match fallback...`,
      );
      ...
      if (words.length > 0) {
        ...
        try {
          const entryRows = memoryVault
            .prepare(selectSql)
            .all(...params) as any[];
          for (const entry of entryRows) {
            const matchTokens =
              entry.token_count ||
              Math.ceil((entry.text_content || "").length / 4);
            if (tokenEstimate + matchTokens <= tokenBudget) {
              results.push({
  ...
  static async getIndexCoverage(): Promise<CoverageReport> {
    ...
    for (const table of tables) {
      try {
        const totalRow = memoryVault
          .prepare(`SELECT count(*) as count FROM ${table}`)
          .get() as any;
        const totalRows = totalRow ? totalRow.count : 0;

        const indexedRow = memoryVault
          .prepare(
            `SELECT count(*) as count FROM semantic_index WHERE source_table = ?`,
          )
          .get(table) as any;
        const indexedRows = indexedRow ? indexedRow.count : 0;

        const lastIndexedRow = memoryVault
          .prepare(
            `SELECT max(indexed_at) as last FROM semantic_index WHERE source_table = ?`,
          )
          .get(table) as any;
        const lastIndexedAt = lastIndexedRow?.last || "N/A";

        const coveragePct =
          totalRows > 0 ? (indexedRows / totalRows) * 100 : 100;
        ...
        report.tables[table] = {
          ...
          lastIndexedAt,
          avgLatencyMs:
            stats.avgLatencyMs > 0 ? Math.round(stats.avgLatencyMs) : 85,
        };
      } catch (err: any) {
        report.tables[table] = {
          totalRows: 0,
  ...
  static watchAndIndex(): void {
    console.log(
      "[SemanticSearch] Attaching real-time incremental codebase and cognitive indexing listeners...",
    );
    ...
    nexusBus.on("MIND_THINK_COMPLETE", async (event: any) => {
      try {
        ...
        if (id) {
          row = memoryVault
            .prepare(`SELECT * FROM mind_log WHERE id = ?`)
            .get(id);
        } else {
          row = memoryVault
            .prepare(`SELECT * FROM mind_log ORDER BY timestamp DESC LIMIT 1`)
            .get();
        }
        if (row) {
          const text =
        ...
        console.error(
          "[SemanticSearch] MIND_THINK_COMPLETE indexing failed:",
          e,
        );
      }
    });
    ...
    nexusBus.on("SWARM_JOB_COMPLETE", async (event: any) => {
      try {
        ...
        if (id) {
          row = memoryVault
            .prepare(`SELECT * FROM swarm_results WHERE id = ? OR task_id = ?`)
            .get(id, id);
        } else {
          row = memoryVault
            .prepare(
              `SELECT * FROM swarm_results ORDER BY timestamp DESC LIMIT 1`,
            )
            .get();
        }
        if (row) {
          const text =
            row.result ||
            [row.builder_output, row.optimizer_synthesis]
              .filter(Boolean)
              .join("\n") ||
            "";
        ...
        console.error(
          "[SemanticSearch] SWARM_JOB_COMPLETE indexing failed:",
          e,
        );
      }
    });
    ...
    nexusBus.on("FILE_MODIFIED", async (event: any) => {
      try {
        ...
        if (id) {
          row = memoryVault
            .prepare(`SELECT * FROM kinetic_logs WHERE id = ?`)
            .get(id);
        } else {
          row = memoryVault
            .prepare(
              `SELECT * FROM kinetic_logs ORDER BY timestamp DESC LIMIT 1`,
            )
            .get();
        }
        if (row) {
    ...
    nexusBus.on("DNA_PROMOTED", async (event: any) => {
      try {
        ...
        if (id) {
          row = memoryVault
            .prepare(`SELECT * FROM dna_archive WHERE id = ?`)
            .get(id);
        } else {
          row = memoryVault
            .prepare(
              `SELECT * FROM dna_archive ORDER BY timestamp DESC LIMIT 1`,
            )
            .get();
        }
        if (row) {
    ...
    nexusBus.on("OVERNIGHT_TASK_COMPLETE", async (event: any) => {
      try {
        ...
        if (id) {
          row = memoryVault
            .prepare(`SELECT * FROM overnight_log WHERE id = ?`)
            .get(id);
        } else {
          row = memoryVault
            .prepare(
              `SELECT * FROM overnight_log ORDER BY started_at DESC LIMIT 1`,
            )
            .get();
        }
        if (row) {
        ...
        console.error(
          "[SemanticSearch] OVERNIGHT_TASK_COMPLETE indexing failed:",
          e,
        );
      }
    });
  ...
  static async indexNewEntry(
    ...
    try {
      ...
      console.error(
        `[SemanticSearch] Failed to index new entry for ${tableName}:${rowId}:`,
        err,
      );
      return false;
    }
  ...
  static getIndexStats() {
    try {
      const row = memoryVault
        .prepare(`SELECT count(*) as count FROM semantic_index`)
        .get() as any;
      const totalIndexed = row ? row.count : 0;
      const lastUpdate = memoryVault
        .prepare(`SELECT max(indexed_at) as last FROM semantic_index`)
        .get() as any;
      return {
        totalIndexed,
