...
echo "[3/4] merge onto original textured mesh..."
bash launch/inference/merge.sh --source "$WORK/skin.fbx" --target "$IN" --output "$WORK/rigged.glb"
echo "[4/4] rename bone_N -> Mixamo names (topology) so it binds to our BONE_MAP..."
node "$(dirname "$0")/rename_bones.cjs" "$WORK/rigged.glb" "$OUT"
echo "rigged + renamed -> $OUT"
