...
void ABannonFighter::ApplyImpact(float Impact){	StunMeter = FMath::Min(100.0f, StunMeter + (Impact * 0.35f));	if (StunMeter >= 100.0f) { bIsStunned = true; StunMeter = 0.0f; }	HP    = FMath::Max(0.0f, HP - Impact * bannon::DMG_SCALE);	Poise = FMath::Max(0.0f, Poise - Impact * 2.0f);	if (Poise <= 0.0f) bCrumpled = true;}
